...
BaseService.addMethod(MessageService.prototype, 'sendMMS', function (CorpNum, Sender, Receiver, ReceiverName, Subject, Contents, FilePaths, BinaryFiles, reserveDT, adsYN, requestNum, success, error) {
    var Messages = {
        Sender: Sender,
        Receiver: Receiver,
        ReceiverName: ReceiverName,
    ...
    this.sendMessage_MMS(CorpNum, '', '', '', '', Messages, FilePaths, BinaryFiles, reserveDT, adsYN, requestNum, success, error);
});
...
BaseService.addMethod(MessageService.prototype, 'sendMessage_MMS', function (CorpNum, Sender, SenderName, Subject, Contents, Messages, FilePaths, BinaryFiles, reserveDT, adsYN, requestNum, success, error) {
    if (!CorpNum || 0 === CorpNum.length) {
        this._throwError(-99999999, '팝빌회원 사업자번호가 입력되지 않았습니다.', error);
        return;
    }
    ...
    for (var i in BinaryFiles) {
        binaryFiles.push({
            fieldName: 'file',
            fileName: BinaryFiles[i].fileName,
            fileData: BinaryFiles[i].fileData,
        });
    }
    ...
    if(binaryFiles.length > 0){
        dto.BinaryFiles = binaryFiles;
    } else {
        dto.Files = files;
    }

    this._executeAction(dto);
});
